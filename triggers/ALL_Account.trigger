trigger ALL_Account on Account (before insert, before update, after insert, after update) 
{
    ...
    if(Trigger.isAfter && (Trigger.isUpdate || Trigger.isInsert)) // insert also
    {
       /* if(Trigger.isInsert || Trigger.isUpdate)
        {
            boolean notAsynchromous = !system.isBatch() && !system.isFuture() && !system.isScheduled();
            boolean recursiveCheck = ALL_CheckRecursive.runCodeBlockOnce('timeZoneWebserviceCall');
            if(notAsynchromous && recursiveCheck)
            {
                CB_Webservices_Handler.beforeTimezoneWebserviceCall(Trigger.new);
            }
        }*/
